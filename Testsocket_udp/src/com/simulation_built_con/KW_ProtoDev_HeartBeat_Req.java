...
class KW_ProtoDev_HeartBeat_Req extends Thread{
	...
	CountDownLatch latch;          //计数器用于统计时间的
	int time=0;                    //用于统计运行的次数
	
	public KW_ProtoDev_HeartBeat_Req(String ucDeviceID,CountDownLatch latch){
		this.latch=latch;
		this.imei=ucDeviceID;
		 ...
		 public void run(){
			 ...
			 while(true){
				 ...
				 if((end_time-start_time)>=10000){
					 System.out.println(this.imei+"has send 10 seconds");
					 latch.countDown();
					 break;
				 }
